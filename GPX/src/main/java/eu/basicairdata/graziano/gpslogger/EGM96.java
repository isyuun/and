...
import static eu.basicairdata.graziano.gpslogger.LocationExtended.NOT_AVAILABLE;
...
class EGM96 {
     ...
     */
    public static EGM96 getInstance(Context c){
        context = c;
        return instance;
    }
    ...
    public void loadGrid(String sharedPath, String privatePath) {
        if (!isEGMGridLoaded && !isEGMGridLoading) {
            ...
            if (sharedPath.startsWith("content"))
                sharedFolder = DocumentFile.fromTreeUri(context, Uri.parse(sharedPath));
            else
    ...
    public boolean isGridAvailable(String path) {
        ...
        try {
            if (path.startsWith("content"))
                gridFolder = DocumentFile.fromTreeUri(context, Uri.parse(path));
        ...
        public void run() {
            ...
            if (!isPrivateDocumentPresent && isSharedDocumentPresent) {
                ...
                if (sharedDocument.exists()) {
                    ...
                    OutputStream out;
                    try {
                        in = context.getContentResolver().openInputStream(sharedDocument.getUri());
                        out = context.getContentResolver().openOutputStream(privateDocument.getUri());
                        copyFile(in, out);
                        in.close();
                        in = null;
                        out.flush();
            ...
            if (isPrivateDocumentPresent) {
                ...
                InputStream fin;
                try {
                    fin = context.getContentResolver().openInputStream(privateDocument.getUri());
